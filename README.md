...
SakugaCut是一个视频结构迁移、生成系统。用户可以上传视频沉淀知识库；在生成视频页面使用知识库（也支持系统自动选择）。

用户可以在生成视频页面上传样例视频，和自己的素材（支持音、视频和图片），系统基于LLM Agent综合分析各个素材的分段-语义信息，调用TTS补充可能缺失的口播，使用hyperframes相关能力剪辑出最终的视频，并给出视频脚本、可能的素材缺失信息以及当前补救策略。

系统支持对产出的视频二次修改，用户仅需自然语言输入修改方案。
...
用户在“生成视频”页面（也是默认进去的页面）可以上传样例视频（可多个,也可以0个）、输入素材（支持多个视频、音频、图片，但是至少有一个）、选择知识库（optional）、填写目标要求（optional），系统根据上述信息剪辑出视频。
...
系统支持对本系统生成的视频用自然语言继续修改。用户输入对视频的修改意见，系统创建一个新job，复用parent job的phase 1产物analysis.json（recall: 素材和样例的分段-语义信息），重跑一遍phase2。在analysis里加入如下handoff信息：
